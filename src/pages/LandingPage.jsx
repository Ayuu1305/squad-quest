...
function LiveMarquee() {
  const stats = [
    ...
    { icon: Crown, text: 'Gold Tier Unlocked: Ayush', color: 'text-yellow-400' },
    ...
    { icon: Sword, text: 'Boss Defeated: Procrastination', color: 'text-red-400' },
  ];
      ...
      >
        {[...stats, ...stats, ...stats].map((stat, index) => (
          <div key={index} className="flex items-center gap-3 mx-8">
            ...
            <span className="text-sm font-['Rajdhani'] font-medium text-white/70 tracking-wide uppercase">{stat.text}</span>
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        <div>
            ...
            <Icon className="w-7 h-7 text-white group-hover:text-[#a855f7] transition-colors" strokeWidth={1.5} />
            </div>
            
            <h3 className="text-2xl font-['Orbitron'] font-bold text-white mb-3 uppercase tracking-wide">{title}</h3>
            <p className="text-white/60 font-['Rajdhani'] text-base leading-relaxed mb-6">{description}</p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      {[1, 2, 3].map((i) => (
        ...
        >
          <div className={`w-5 h-5 rounded-full flex items-center justify-center ${i <= 2 ? 'bg-green-500 text-black' : 'bg-white/10'}`}>
             {i <= 2 && <motion.div initial={{scale:0}} animate={{scale:1}} className="text-[10px] font-bold">✓</motion.div>}
          </div>
          <span className={`text-sm font-['Rajdhani'] ${i <= 2 ? 'text-white' : 'text-gray-500'} font-medium`}>
            {i === 1 ? 'Daily Objective Alpha' : i === 2 ? 'Side Quest: Focus' : 'Weekly Challenge'}
          </span>
        </motion.div>
      ))}
    </div>
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4 mb-4 relative z-10">
          ...
          <div>
            <p className="text-yellow-400 font-['Orbitron'] font-bold text-sm tracking-wider">GOLD TIER</p>
            <p className="text-white/60 text-xs font-['Rajdhani'] uppercase">Sector 4 • Rank #3</p>
          </div>
        </div>
...
function BadgeCollection() {
  const badges = [
    { icon: Flame, color: 'from-orange-500 to-red-600', ring: 'ring-orange-500/30' },
    { icon: Trophy, color: 'from-yellow-400 to-amber-600', ring: 'ring-yellow-500/30' },
    { icon: Target, color: 'from-cyan-400 to-blue-600', ring: 'ring-cyan-500/30' },
    { icon: Shield, color: 'from-purple-500 to-fuchsia-600', ring: 'ring-purple-500/30' },
  ];
  return (
    ...
    <div className="flex items-center justify-center mt-6 h-12 relative">
       {[0, 1, 2].map((i) => (
         <motion.div
            ...
            transition={{ delay: i * 0.2 }}
         >
            <div className={`w-full h-full bg-gradient-to-br ${i===0 ? 'from-purple-500 to-blue-500' : i===1 ? 'from-blue-500 to-cyan-500' : 'from-cyan-500 to-green-500'} opacity-80`} />
         </motion.div>
       ))}
...
function RewardExamples() {
    const examples = [
        { text: "FREE COFFEE", color: "text-orange-400", border: "border-orange-400/30", bg: "bg-orange-400/10" },
        { text: "10% OFF GAMING", color: "text-purple-400", border: "border-purple-400/30", bg: "bg-purple-400/10" },
        { text: "GYM DAY PASS", color: "text-green-400", border: "border-green-400/30", bg: "bg-green-400/10" },
    ];
    return (
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Icon size={40} style={{ color: colorHex }} className="relative z-10 drop-shadow-[0_0_10px_rgba(0,0,0,0.5)]" />
      </div>
      <span className="font-['Orbitron'] text-white/80 tracking-widest text-xs sm:text-sm uppercase group-hover:text-white transition-colors">{label}</span>
    </motion.div>
  )
}
        ...
        <main>
          ...
          <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
             ...
             <div className="relative z-20 text-center px-4 max-w-5xl mx-auto pt-24 md:pt-0">
                <motion.div initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} transition={{delay:0.2}} className="mb-8 inline-block">
                ...
                <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black font-['Orbitron'] mb-6 leading-none tracking-tighter drop-shadow-2xl">
                  <TextReveal text="CONQUER" delay={0.3} className="block text-white" />
                ...
                <p className="text-gray-300 text-lg md:text-2xl max-w-2xl mx-auto mb-12 font-light font-['Rajdhani'] leading-relaxed">
                   Transform daily chaos into epic quests. Climb the leaderboard, unlock rare loot, and become the main character of your life.
                </p>
                ...
                {/* Stats Footer */}
                <div className="mt-20 grid grid-cols-3 gap-8 max-w-lg mx-auto border-t border-white/10 pt-8 opacity-80">
                   {[
                     { val: '50K+', label: 'Heroes' },
                     { val: '2M+', label: 'Quests' },
                     { val: '10K+', label: 'Rewards' }
                   ].map((s, i) => (
                      <div key={i}>
                         <div className="text-2xl font-bold font-['Orbitron'] text-white">{s.val}</div>
                         <div className="text-[10px] text-gray-400 font-['Rajdhani'] uppercase tracking-widest">{s.label}</div>
                      </div>
                   ))}
                </div>
             </div>
          ...
          <section className="py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
             <div className="text-center mb-20">
                ...
                <p className="text-gray-400 font-['Rajdhani'] text-lg">Our system tracks your real-world achievements and converts them into digital glory.</p>
             </div>
          ...
          <section className="py-32 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#0f0f23] to-[#0a0a1a] relative overflow-hidden">
             ...
             <div className="relative max-w-7xl mx-auto text-center">
                ...
                <h2 className="text-4xl md:text-6xl font-['Orbitron'] font-black text-white mb-8">
                   YOUR XP <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">PAYS OFF</span>
                </h2>
                <p className="text-gray-400 font-['Rajdhani'] text-xl max-w-2xl mx-auto mb-20">
                   Exchange Quest Points for vouchers at your favorite local spots. Your productivity now has real value.
                </p>
                ...
                <div className="mt-20">
                   ...
                   <button onClick={handleStart} className="px-10 py-4 rounded-full font-['Orbitron'] font-bold bg-white text-[#0f0f23] hover:scale-105 transition-transform shadow-[0_0_40px_rgba(255,255,255,0.2)]">
                      Explore Rewards
                   </button>
                </div>
             </div>
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
         ...
         <div className="col-span-1 md:col-span-2">
            <div className="flex items-center gap-3 mb-6">
               ...
               <span className="font-['Orbitron'] font-bold tracking-widest text-xl text-white">SQUAD QUEST</span>
            </div>
            <p className="text-gray-400 font-['Rajdhani'] mb-6 max-w-sm leading-relaxed">
               The ultimate gamified productivity platform. Turn your life into an RPG, compete with friends, and earn real-world rewards.
            </p>
            <div className="flex items-center gap-4">
               <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-[#a855f7]/20 hover:text-white transition-all group">
         ...
         {/* Links Column */}
         <div>
            <h4 className="font-['Orbitron'] font-bold text-white mb-6 uppercase tracking-wider">Quick Links</h4>
            <ul className="space-y-3 font-['Rajdhani'] text-gray-400">
               <li><a href="#" className="hover:text-[#a855f7] transition-colors flex items-center gap-2"><ChevronRight className="w-3 h-3 text-[#a855f7]" />Features</a></li>
               <li><a href="#" className="hover:text-[#a855f7] transition-colors flex items-center gap-2"><ChevronRight className="w-3 h-3 text-[#a855f7]" />Rewards</a></li>
               <li><a href="#" className="hover:text-[#a855f7] transition-colors flex items-center gap-2"><ChevronRight className="w-3 h-3 text-[#a855f7]" />Leaderboard</a></li>
               <li><a href="#" className="hover:text-[#a855f7] transition-colors flex items-center gap-2"><ChevronRight className="w-3 h-3 text-[#a855f7]" />About Us</a></li>
            </ul>
         </div>

         {/* Newsletter Column (Visual Only) */}
         <div>
            <h4 className="font-['Orbitron'] font-bold text-white mb-6 uppercase tracking-wider">Join The Squad</h4>
            <p className="text-gray-400 font-['Rajdhani'] mb-4 text-sm">Subscribe for updates, new quests, and exclusive reward drops.</p>
            <div className="flex">
               <input type="email" placeholder="Enter email address" className="bg-white/5 border border-white/10 rounded-l-xl px-4 py-3 text-sm font-['Rajdhani'] text-white focus:outline-none focus:border-[#a855f7]/50 w-full" />
