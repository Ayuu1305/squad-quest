...
const CompetitionsList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setCompetitions(
          snapActive.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
        );
        ...
        setHistory(
          snapHistory.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
        );
      } catch (error) {
  ...
  const handleJoinClick = (war) => {
        ...
        (t) => (
          <div className="flex flex-col gap-2 p-2">
            ...
            <span className="text-xs text-gray-300">
              Add your college in profile to join the war.
            </span>
            <button
              onClick={() => {
                toast.dismiss(t.id);
              ...
              className="bg-neon-purple text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-neon-purple/80 transition-all"
            >
              Go to Profile
            </button>
          </div>
    ...
    if (!war.colleges.includes(userCollege)) {
      toast.error(`🚫 Access Denied: "${userCollege}" is not participating.`, {
        duration: 4000,
        style: {
          background: "#1a1a2e",
          color: "#fff",
          ...
          <>
            {competitions.length === 0 ? (
              ...
              competitions.map((war) => {
                const userCollege = user?.college?.trim();
                const hasJoined =
                  userCollege && war.colleges.includes(userCollege);

                return (
                  <div
                    key={war.id}
                    className="relative glassmorphism border border-white/10 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(168,85,247,0.2)]"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-neon-purple/20 via-transparent to-red-500/20" />
                    <div className="relative z-10 p-8 space-y-6">
                      <div className="text-center space-y-3">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-500/20 border border-red-500/30 rounded-full">
                          <Flame className="w-4 h-4 text-red-500 animate-flame-glow" />
                          <span className="text-[10px] font-black text-red-500 uppercase tracking-[0.3em]">
                            Live Battle
                          </span>
                        </div>
                        <h2 className="text-3xl md:text-4xl font-['Orbitron'] font-black text-white italic uppercase tracking-tighter leading-tight">
                          {war.title}
                        </h2>
                        <p className="text-gray-400 text-xs uppercase tracking-widest font-black">
                          Season 1 • Inter-College XP Battle
                        </p>
                      </div>
                      ...
                      <div className="bg-black/40 border border-yellow-500/30 rounded-2xl p-6">
                        ...
                        <div className="text-center">
                          <div className="text-4xl font-black font-['Orbitron'] text-yellow-500 italic mb-2">
                            ₹5,000
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-black/40 border border-white/10 rounded-xl p-4">
                          ...
                          <div className="text-lg font-black font-['Orbitron'] text-green-500 italic flex items-center gap-2">
                      ...
                      >
                        ...
                        {hasJoined ? (
                          <>
                            ...
                            <span className="uppercase tracking-widest text-xs md:text-sm italic font-['Orbitron'] relative z-10">
                              Enter War Room
                            </span>
            ...
            >
              <h3 className="text-center text-sm font-black text-white uppercase tracking-widest">
                How It Works
              </h3>
              <div className="grid gap-3">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-3">
                  <div className="w-8 h-8 bg-neon-purple/20 border border-neon-purple rounded-full flex items-center justify-center flex-shrink-0">
                    <span className="text-neon-purple font-black text-xs">
                      1
                    </span>
                  </div>
                  <div>
                    ...
                    <div className="text-gray-500 text-[10px] leading-relaxed">
                      Earn XP by completing any quest in Ahmedabad
                    </div>
                  </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-[10px] leading-relaxed">
                      Your XP automatically adds to your college's total score
                    </div>
                  </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-[10px] leading-relaxed">
                      College with most XP wins the prize pool at week's end
                    </div>
                  </div>
                </div>
              </div>
              ...
              history.map((war) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex-1">
                    <h3 className="text-white font-bold font-['Orbitron'] uppercase italic tracking-wide text-sm">
                      {war.title}
                    </h3>
